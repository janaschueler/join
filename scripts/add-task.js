...
function addInputButtons(inputField) {
    const buttonContainer = document.createElement("div");
    buttonContainer.className = "input-button-container";
    ...
    inputField.parentNode.insertBefore(buttonContainer, inputField);
}

function removeInputButtons(inputField) {
    const buttonContainer = inputField.parentNode.querySelector(".input-button-container");
    if (buttonContainer) {
        buttonContainer.remove();
    }
}

function addNewSubtaskElement(text) {
    const container = document.getElementById("subtasks-container");
    const textDiv = document.createElement("div");
    textDiv.classList.add("subtask-text-item");
    textDiv.textContent = "• " + text;
    textDiv.dataset.type = ''; // Kein Typ mehr benötigt
    ...
    deleteButton.onclick = function() { deleteSubtaskText(this); };
...
function addSubtask() {
    ...
    if (!inputField.classList.contains("plus-active")) {
        inputField.classList.add("plus-active");
        // Vorschlag basierend auf Klick-Zähler
        if (subtaskClickCount === 0) {
            inputField.value = "Contact form"; // Geänderter Vorschlag
        } else if (subtaskClickCount === 1) {
            inputField.value = "Write legal imprint";  // Geänderter Vorschlag
        }
        subtaskClickCount++; // Erhöhe den Zähler *nachdem* der Vorschlag gesetzt wurde
    }

    // Füge Buttons nur hinzu, wenn sie noch nicht da sind
    if (!inputField.parentNode.querySelector(".input-button-container")) {
        addInputButtons(inputField);
    }
}


function deleteSubtaskText(button) {
    button.parentElement.remove(); // Einfaches Löschen, da keine Typen mehr
}
...
function addTask() {
    ...
    const assignedContacts = selectedContactIds.map(contactId => {
